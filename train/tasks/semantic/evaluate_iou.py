...
if __name__ == '__main__':
    ...
    # create evaluator
    class_ids_to_ignore = []
    for class_id, should_ignore in class_ignore.items():
        if should_ignore:
            x_cl = int(class_id)
            class_ids_to_ignore.append(x_cl)
            print("Ignoring xentropy class ", x_cl, " in IoU evaluation")

    # create evaluator
    device = torch.device("cpu")
    evaluator = IoUEval(number_of_classes, device, class_ids_to_ignore)
    evaluator.reset()
    ...
    for scan_file, label_file, pred_file in zip(scan_names, label_names, pred_names):
        ...
        evaluator.add_batch(u_pred_sem, u_label_sem)
    ...
    for i, jacc in enumerate(class_jaccard):
        if i not in class_ids_to_ignore:
